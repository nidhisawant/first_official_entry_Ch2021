function [features, fbfeat,  pentropyTrain, instfreqTrain, scattTrain] = get_features(data, header_data,leads_idx) %get_ECGLeads_features
...
try
   ...
   scattTrain=[];
    for kk=[leads_idx]
            pentropyTrain(kk,:)=pentropy(data1(kk,:),Fs)';
            instfreqTrain(kk,:)=instfreq(data1(kk,:),Fs)';
            sct_feat= featureMatrix(sf,data1(kk,:))';
    ...
    pentropyTrain=zeros(features_length,129);
    instfreqTrain=zeros(features_length,129);
    scattTrain=zeros(features_length*10,202);
    fbfeat=zeros(1,20);
end
